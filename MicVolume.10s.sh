...
function show_mic_volume() {
    echo "$(get_current_device | cut -c 1-6)($(get_mic_volume)) | size=16
---
Mic Volume
---
マイク音量最小化(F7) | bash='$0' param1=minimize_mic_volume terminal=false shortcut=F7 refresh=true
マイク音量最大化(F8) | bash='$0' param1=maximize_mic_volume terminal=false shortcut=F8 refresh=true
---"
    list_input_device | while read line;do
	if [ "$line" != "ZoomAudioDevice" -a "$line" != "IOUSBHostInterface" ];then
	    echo "$line | bash='$0' param1=change_input_device param2='$line' terminal=false refresh=true"
	fi
    done
}
